...
public class ArcadeMachine {
     ...
     */
    public static double[] playOneGame(String game_file, String level_file, String actionFile, int randomSeed,double[] solution) {
		String agentName = "tracks.singlePlayer.tools.human.Agent";
		boolean visuals = true;
		return runOneGame(game_file, level_file, visuals, agentName, actionFile, randomSeed, 0,solution);
    }
     ...
     */
    public static double[] playOneGameMulti(String game_file, String level_file, String actionFile, int randomSeed,double[] solution) {
    ...
    public static double[] runOneGame(String game_file, String level_file, boolean visuals, String agentNames,
	    String actionFile, int randomSeed, int playerID,double[] solution) {
		...
		for (int i = 0; i < no_players; i++) {
			...
			players[i] = ArcadeMachine.createMultiPlayer(names[i], actionFile, toPlay.getObservationMulti(i),
				randomSeed, i, humans[i],solution);
			} else {
			// single player
     ...
     *
     */
    public static double[] replayGame(String game_file, String level_file, boolean visuals, String actionFile,double[] solution) {
		VGDLFactory.GetInstance().init(); // This always first thing to do.
		VGDLRegistry.GetInstance().init();
		...
		for (int i = 0; i < no_players; i++) {
			...
			// single player
			players[i] = ArcadeMachine.createPlayer(agentName, null, toPlay.getObservation(), -1, false,solution);
			}
     ...
     */
    public static void runGames(String game_file, String[] level_files, int level_times, String agentName, String[] actionFiles,double[] solution) {
	VGDLFactory.GetInstance().init(); // This always first thing to do.
	VGDLRegistry.GetInstance().init();
	...
	for (String level_file : level_files) {
		...
		for (int j = 0; j < no_players; j++) {
			...
			players[j] = ArcadeMachine.createMultiPlayer(agentNames[j], filename,
				toPlay.getObservationMulti(i), randomSeed, j, false,solution);
		    } else {
			// single player
			players[j] = ArcadeMachine.createPlayer(agentNames[j], filename, toPlay.getObservation(),
				randomSeed, false,solution);
		    }
		    score[j] = -1;
		    if (players[j] == null) {
				// Something went wrong in the constructor, controller
    ...
    public static AbstractPlayer createPlayer(String playerName, String actionFile, StateObservation so,
	    int randomSeed, boolean isHuman, double[] solution) {
        AbstractPlayer player = null;

        try {
            // create the controller.
            player = (AbstractPlayer) createController(playerName, 0, so, solution);
            if (player != null)
    ...
    public static AbstractMultiPlayer createMultiPlayer(String playerName, String actionFile, StateObservationMulti so,
	    int randomSeed, int id, boolean isHuman, double[] solution) {
        AbstractMultiPlayer player = null;

        try {
            // create the controller.
            player = (AbstractMultiPlayer) createController(playerName, id, so,solution);
            if (player != null) {
            player.setup(actionFile, randomSeed, isHuman);
            }
    ...
    protected static Player createController(String playerName, int playerID, StateObservation so,double[] solution)
        ...
        try {
            ...
            if (so.getNoPlayers() < 2) { // single player
				...
				Class[] gameArgClass = new Class[] { StateObservation.class, ElapsedCpuTimer.class,double[].class };
				...
				Object[] constructorArgs = new Object[] { so, ect.copy(), solution };
